...
// Onshape auto-assigns a part number like "MOD-ASM-14" when a part has no
// custom part number configured. The BOM API doesn't flag which is which,
// so this is a heuristic: Onshape's auto-generated shape is consistently
// "WORD-WORD-digits" (three hyphen-separated segments, last one numeric).
// Real part numbers with 2 segments (WCP-0733) or numeric-numeric
// (217-4006) won't match. Override via ONSHAPE_AUTO_PART_NUMBER_REGEX in
// .env if your real part numbers happen to collide with this shape.
const AUTO_PART_NUMBER_PATTERN = process.env.ONSHAPE_AUTO_PART_NUMBER_REGEX
  ? new RegExp(process.env.ONSHAPE_AUTO_PART_NUMBER_REGEX)
  : /^[A-Za-z]+-[A-Za-z]+-\d+$/

function isAutoGeneratedPartNumber(pn) {
  return !!pn && AUTO_PART_NUMBER_PATTERN.test(pn)
}
...
export function resolveRow(row, headerById) {
  ...
  const rawResolvedPartNumber = unwrapBomValue(rawPartNumber) ? String(unwrapBomValue(rawPartNumber)) : ''
  const partNumber = isAutoGeneratedPartNumber(rawResolvedPartNumber) ? '' : rawResolvedPartNumber
